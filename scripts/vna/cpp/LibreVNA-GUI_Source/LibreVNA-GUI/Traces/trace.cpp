...
QString Trace::getMathFormulaError() const
{
    ...
    try {
        ...
        parser.SetExpr(to_mup_string(mathFormula));
        auto vars = parser.GetExprVar();
        for(auto var : vars) {
            auto varName = from_mup_string(var.first);
        ...
        return "Parsing failed: " + from_mup_string(e.GetMsg());
    }
    ...
    if(!isPaused()) {
        try {
            ...
            for(const auto &ts : mathSourceTraces) {
                ...
                parser.DefineVar(to_mup_string(ts.second), Variable(&values[ts.first]));
            }
