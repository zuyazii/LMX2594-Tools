...
static bool driver_debug_enabled()
{
    static int cached = -1;
    if (cached == -1) {
...
static void driver_raw_dbg(const char *msg)
{
    if (!driver_debug_enabled()) return;
    fprintf(stderr, "[LibreVNADriver] %s\n", msg);
    fflush(stderr);
}
...
void LibreVNADriver::handleReceivedPacket(const Protocol::PacketInfo &packet)
{
    if (driver_debug_enabled()) {
        char msg[256];
        snprintf(msg, sizeof(msg), "handleReceivedPacket type=%d", (int)packet.type);
        driver_raw_dbg(msg);
    }
    emit passOnReceivedPacket(packet);

    if(skipOwnPacketHandling) {
        driver_raw_dbg("handleReceivedPacket: skipOwnPacketHandling=true, returning");
        return;
    }

    switch(packet.type) {
    case Protocol::PacketType::DeviceInfo: {
        driver_raw_dbg("handleReceivedPacket: DeviceInfo received!");
        ...
        driver_raw_dbg("handleReceivedPacket: emitting InfoUpdated");
        emit InfoUpdated();
        driver_raw_dbg("handleReceivedPacket: InfoUpdated emitted");
    }
        break;
