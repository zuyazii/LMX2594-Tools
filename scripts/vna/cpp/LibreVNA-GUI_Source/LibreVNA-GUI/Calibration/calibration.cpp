...
{
    qDebug() << "fromJSON: reset()";
    reset();
    qDebug() << "fromJSON: acquiring lock";
    lock_guard<recursive_mutex> guard(access);
    qDebug() << "fromJSON: lock acquired";
    if(j.contains("calkit")) {
        qDebug() << "fromJSON: loading calkit";
    ...
    qDebug() << "fromJSON: format=" << format;
    switch(format) {
    case 3: {
        if(j.contains("measurements")) {
            qDebug() << "fromJSON: loading measurements";
            ...
            qDebug() << "fromJSON: loaded" << measurements.size() << "measurements";
        }
        ...
        if(ct.type != Type::None) {
            qDebug() << "fromJSON: calling compute()";
            compute(ct);
            qDebug() << "fromJSON: compute() returned";
        }
...
bool Calibration::fromFile(QString filename)
{
    ...
    qDebug() << "File opened, parsing JSON...";

    try {
        nlohmann::json j;
        file >> j;
        qDebug() << "JSON parsed, calling fromJSON...";
        currentCalFile = filename;    // if all ok, remember this
        fromJSON(j);
        qDebug() << "fromJSON completed";
    } catch(exception &e) {
